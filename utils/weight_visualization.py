...
def weight_visulize_py(smiles, atom_weight,tasks_index):
    ...
    smiles_name = eval(repr(smiles).replace('/', '*'))
    #MolToFile(mol, r'./A/'+smiles_name+'.png', highlightAtoms=range(0, mol.GetNumAtoms()), highlightBonds=range(0, mol.GetNumBonds()),
              #highlightAtomColors=atom_colors, highlightBondColors=bond_colors)
    drawer.DrawMolecule(mol, highlightAtoms=range(0, mol.GetNumAtoms()), highlightBonds=range(0, mol.GetNumBonds()),
                        highlightAtomColors=atom_colors, highlightBondColors=bond_colors)
    drawer.FinishDrawing()
    svg = drawer.GetDrawingText()
    with open(r'./mutiB/'+smiles_name+'.svg','w') as f:
        f.write(svg)
