...
for time_id in range(args['times']):
    ...
    model = MGA(in_feats=args['in_feats'], rgcn_hidden_feats=args['rgcn_hidden_feats'],
                   n_tasks=1, rgcn_drop_out=args['rgcn_drop_out'],
    ...
    selected_data = pd.read_csv('data/KOWall3_group.csv')
    selected_mol_list = selected_data['smiles'].tolist()


    # visual test set
run_an_eval_epoch_heterogeneous_return_weight_py2(args, model, test_loader, vis_list=selected_mol_list)
run_an_eval_epoch_heterogeneous_return_weight_py2(args, model, train_loader, vis_list=selected_mol_list)
run_an_eval_epoch_heterogeneous_return_weight_py2(args, model, val_loader, vis_list=selected_mol_list)
